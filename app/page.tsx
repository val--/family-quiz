...
export default async function Home() {
  return (
    <div className="flex flex-1 items-center justify-center bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500 p-2 min-h-0">
            ...
            quizList.map((quiz) => (
              <Link key={quiz.id} href={`/quiz/${quiz.id}`}>
                <div className="relative flex flex-col items-center justify-center text-purple-900 font-semibold w-48 h-48 transition-transform transform hover:scale-110">
                  ...
                  />
                  <span className="text-xl z-10">{quiz.theme}</span>
                  <p className="text-white mt-2 hidden">
                    {quiz.id ? `Dernier score: ${quiz.id}` : "Pas de score"}
                  </p>
                </div>
              </Link>
            ))}
